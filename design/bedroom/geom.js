...
const POOL_OCC_TALL = {
  'bd-laptop': [31, 43],      // 노트북
  'bd-deskplant': [22, 26],   // 카페인 음료(유리와 겹치는 부분만)
  'bd-lamp': [46, 50],        // 스탠드
  'bd-desk': [28, 29],        // 콜라캔(책상 그룹 소품)
};
const TALL_Y1 = 62;
...
// ── 책상 스탠드 — 밤 작업 조명. 책상 오른끝에서 **두 칸 안쪽**(x45~50) —
// 모서리에 걸터앉은 것처럼 보이지 않게. 받침이 상판(y35~36)에 닿는다.
export function lampArt() {
  return [
    R(46, 27, 5, 1, '#8a6a3a'), R(45, 28, 6, 2, '#6e5230'), R(45, 30, 6, 1, '#4e3a22'),
    R(47, 30, 3, 1, '#c9a86a'), R(48, 31, 1, 4, '#4a4150'),
    R(46, 35, 4, 1, '#3a3242'), R(46, 36, 4, 1, '#241f2e'),
  ];
}
export function lampGlowArt() {
  return [
    R(47, 30, 3, 2, '#fff1c0'),
    R(45, 29, 7, 5, '#ffd98a', 0.5),
    R(42, 27, 13, 9, '#ffcf80', 0.24),
    R(39, 25, 19, 13, '#ffc266', 0.1),
    // 상판에 떨어지는 빛 웅덩이 — 표면에 닿아야 켜진 걸로 읽힌다(거실 점광 대응)
    R(43, 35, 9, 1, '#ffd98a', 0.22),
    R(42, 36, 11, 1, '#ffcf80', 0.14),
    R(41, 37, 12, 1, '#ffc266', 0.08),
    // 바닥으로 새는 웜 스필(레퍼런스 night-lamp)
    R(46, 49, 12, 1, '#ffcf80', 0.1), R(48, 50, 12, 1, '#ffc266', 0.07),
  ];
}
...
export function screenGlowArt() {
  return [
    R(32, 29, 10, 4, '#89b4e2', 0.85),                    // 화면 코어
    R(33, 29, 6, 1, '#cfe2f4'), R(34, 31, 7, 1, '#b6d2ec'),   // 텍스트 줄
    R(30, 27, 14, 9, '#5f88b8', 0.2),                     // 주변 번짐
    R(29, 35, 16, 2, '#6f9ac8', 0.14),                    // 데크에 비침
  ];
}
