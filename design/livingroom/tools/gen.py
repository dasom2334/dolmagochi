"""레퍼런스 이미지 → 방 구조 **측정**. 추출만 한다.

절차 생성(하늘·산·바닥·러그·돌·나무·광원)은 scene/generate.js 가 런타임에 만든다.
여기 있는 것: 셀 격자 마스크 · 리전별 k-means 양자화 · 인페인팅 · rect 방출.
출력(art_svg_v2.txt / palette_css_v2.txt)은 export_room.py 가 읽어 room-data.js 로 만든다.

  python3 gen.py refgrid_v3.npy _v2 && python3 export_room.py
"""
