...
//
// **옆모습(프로필)**. 지면을 정면으로 펼쳐 보인 판은 각도를 어떻게 바꿔도
// 계속 안 나왔다 — 돌 앞을 지면이 막아 버려서 '읽는 중'이 아니라 '가려진 돌'이
// 됐기 때문이다. 그래서 책을 90° 돌려 **끝에서 본다**:
//   책등(d)이 아래에 모이고 두 쪽이 위로 V 로 벌어진다.
//   각 쪽은 종이 뭉치의 **두께**로 보이므로, 바깥은 표지(C)·안은 지면(P/p)의
//   쐐기 모양이 된다. 가운데 빈 곳은 펼쳐진 틈 — 메우면 그냥 삼각형이다.
// 이 실루엣은 글줄이 하나도 없어도 책으로 읽힌다. 정면 판이 글줄·책등·종이 단을
// 다 갖추고도 안 읽혔던 것과 반대다 — **형태가 디테일보다 먼저다**.
...
function openBook(n) {
  ...
  OPENBOOK_ART.forEach((row, j) => {
    ...
    while (i < row.length) {
      ...
      // 옆모습이라 돌 앞을 막지 않는다 → 돌 **왼쪽 앞**에 세운다(x35~45).
      // 책등이 y63 에서 담요에 닿고 V 는 y55 까지 열린다.
      out.push([35 + i, 55 + j, k - i + 1, 1, map[ch]]);
      i = k + 1;
    }
  });
  return out;
