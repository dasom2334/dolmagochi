...
//
// **옆모습(프로필)** — 돌이 오른쪽을 보고 읽는 자세에 맞춘 각도.
// 지면을 정면으로 펼쳐 보인 판들이 계속 안 나온 건 각도가 아니라, **지면이 돌
// 앞을 막아** '읽는 중'이 아니라 '가려진 돌'이 됐기 때문이다. 90° 돌려 끝에서
// 보면 책등(d)이 아래에 모이고 두 쪽이 위로 벌어지며, 각 쪽은 종이 뭉치의
// **두께**(바깥=표지 C, 안=지면 P/p)로 보인다. 가운데 빈 곳은 펼쳐진 틈이다.
//
// 좌우 대칭 V 는 읽히긴 해도 나비처럼 보인다. 그래서 **왼쪽(돌 쪽)으로 기울인다**:
//   왼쪽 쪽은 위로 갈수록 왼쪽으로 눕고, 오른쪽 쪽은 거의 선 채로 둔다.
//   기울면 대칭이 깨져 '세워 둔 것'이 아니라 '들고 읽는 것'이 된다.
// 앞서 정수 픽셀 쉬어로 기울여 봤다가 쐐기가 행마다 2px 씩 끊겨 조각났다 —
// 이 크기에서는 **행마다 직접 그려** 위아래 행이 최소 2px 겹치게 해야 이어진다.
//   P=지면 p=종이 단 d=책등 C=표지
// 13×9.
const OPENBOOK_ART = [
  'CPp.......pPC',
  '.CPp......pPC',
...
function openBook(n) {
  ...
  OPENBOOK_ART.forEach((row, j) => {
    ...
    while (i < row.length) {
      ...
      // 돌이 **오른쪽을 보고 읽는** 그림이므로 책은 돌 오른쪽(x48~60)에 두고 왼쪽으로 기울인다.
      // 책등이 y63 에서 담요에 닿고 V 는 y55 까지 열린다.
      out.push([48 + i, 55 + j, k - i + 1, 1, map[ch]]);
      i = k + 1;
    }
  });
  return out;
