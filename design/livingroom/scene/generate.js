...
//
// **옆모습(프로필)** — 책등(d)이 아래에 모이고 두 쪽이 위로 V 로 벌어진다.
// 각 쪽은 종이 뭉치의 **두께**(바깥=표지 C, 안=지면 P/p)로 보인다.
// 가운데 빈 곳은 펼쳐진 틈 — 메우면 그냥 삼각형이다.
// 돌이 오른쪽을 보고 읽는 그림이므로 **돌 오른쪽**에 둔다. 돌은 무안면이라
// 스스로 방향을 못 가리키므로, 책의 위치가 곧 돌의 시선이 된다.
//
// (한 번 위로 갈수록 왼쪽으로 기울여 봤는데, 정수 픽셀 쉬어가 쐐기를 2px 씩
//  끊어 조각내 버렸다 — 이 크기에서 기울이려면 행마다 새로 그려야 한다.)
...
function openBook(n) {
  ...
  OPENBOOK_ART.forEach((row, j) => {
    ...
    while (i < row.length) {
      ...
      // 돌이 **오른쪽을 보고 읽는** 그림이므로 책은 돌 오른쪽(x50~60)에 둔다.
      // 책등이 y63 에서 담요에 닿고 V 는 y55 까지 열린다.
      out.push([50 + i, 55 + j, k - i + 1, 1, map[ch]]);
      i = k + 1;
    }
  });
  return out;
