...
// ── 주전자 아랫배 반사광 ────────────────────────────────────────────────
// 화구 링만 깔았더니 **밤에 주전자가 평평했다** — 방 안 유일한 광원이 바로 밑에
// 있는데 몸통이 안 달아오르면 주전자가 빛 위에 오려 붙인 것으로 보인다.
// 불은 아래에서 달구므로 밑변이 제일 밝고 위로 갈수록 죽는다(§3.2 역제곱).
// 알베도에 굽지 않고 여기서 낸다(§2) — 화구를 끄면 같이 사라져야 하기 때문이다.
// ※ 셰이프는 손작화 주전자(x53~73, 밑변 y38) 기준. 시안 A 추출본은 밑변이
//   한 칸 낮지만 링에 묻히는 정도라 따로 두지 않았다.
// 1차는 줄마다 폭이 같아 **곧은 밝은 띠**가 됐다 — 주전자가 형광등 위에 앉은 꼴이다.
// 배가 둥그니 가운데가 밝고 옆구리로 갈수록 죽어야 한다 → 줄마다 중심/옆구리를 나눈다.
const POT_GLOW = [
  [60, 38, 8, '#ffcf8c', 0.30], [58, 38, 2, '#ffb45c', 0.18], [68, 38, 2, '#ffb45c', 0.18],
  [59, 37, 10, '#ffb45c', 0.22], [56, 37, 3, '#ff9440', 0.12], [69, 37, 2, '#ff9440', 0.12],
  [59, 36, 10, '#ff9440', 0.12], [56, 36, 3, '#ff9440', 0.06], [69, 36, 2, '#ff9440', 0.06],
  [60, 35, 8, '#ff9440', 0.05],     // 여기서 끊긴다 — 몸통 위까지 올리면 등불이 된다
  [53, 36, 3, '#ff9440', 0.10],     // 왼쪽 손잡이 밑
  [70, 34, 3, '#ff9440', 0.08],     // 오른쪽 주둥이 밑
];
