...
export const BALANCE = {
  ...
  /**
   * 선택지 무응답 유형 판정 (M25) — 포섀도 슬롯이 하나뿐이라, 뜬 선택지를 계속
   * 무시하면 그 하나가 자리를 물고 있어 나머지 복선이 영영 안 나온다.
   * 최근 CHOICE_WINDOW회 제시 중 선택률이 CHOICE_IGNORE_RATE 미만이면
   * '응답하지 않는 유형'으로 보고, 그 사람에게만 포섀도를 흘려보낸다.
   * 표본이 CHOICE_MIN_SAMPLE에 차기 전에는 절대 판정하지 않는다 — 짧은 세션만
   * 해서 선택지를 몇 번 못 본 사람을 오판하지 않기 위해.
   */
  CHOICE_WINDOW: 20,
  CHOICE_MIN_SAMPLE: 20,
  CHOICE_IGNORE_RATE: 0.1,
  ...
  // 계절별 날씨 가용성·자연 변화 확률 (달력일당 1회 추첨). 표에 없는 날씨는
  // 그 계절에 선택 불가 — 이 표가 '고를 수 있는 목록'과 '안 건드리면 오는 것'을 겸한다.
  //
  // 사계절 공통 5종(맑음·흐림·안개·비·장대비)은 어느 계절에서나 고를 수 있다(M22).
  // 다만 자연 확률은 계절색을 지킨다 — 장대비가 한겨울에 여름만큼 오면 계절감이
  // 무너지므로 여름 밖에서는 .05로 둔다. 고르면 언제든 오지만 저절로 오는 일은 드물다.
  // 안개는 어느 계절에나 드문 날씨라 낮게, 가을만 조금 높다.
  // 계절 전용은 꽃잎비=봄, 풀잎비=여름, 낙엽비=가을, 눈=겨울.
  // ⚠ 계절 전용을 배열 끝에 두는 순서는 누적 추첨 테스트가 의존한다.
  WEATHER_BY_SEASON: {
    spring: [
      ['clear', 0.35],
      ['cloud', 0.12],
