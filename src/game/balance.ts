...
export const BALANCE = {
  ...
  //
  // 비·장대비는 네 계절 모두 선택 가능(M22). 다만 자연 확률은 계절색을 지킨다:
  // 장대비가 한겨울에 여름만큼 오면 계절감이 무너지므로, 여름 밖에서는 .05로 둔다
  // (고르면 언제든 오지만, 저절로 오는 일은 드물다). 계절 전용은 꽃잎비=봄,
  // 풀잎비=여름, 낙엽비=가을, 눈=겨울로 그대로.
  WEATHER_BY_SEASON: {
    spring: [
      ['clear', 0.45],
      ['rain', 0.2],
