...
// [자리를 두 번 옮겼다]
// · 작업대 위(cx66)는 **냄비 바로 뒤**라 겹쳐 보였다 → 싱크 상판 왼쪽 끝으로.
//   개수통 왼쪽은 원래 비어 있던 자리고, 창 볕이 드는 자리라 역광도 잘 잡힌다.
...
// 여기선 방의 돌 자리를 좌표로 넘겨주기만 한다.
export const sproutArt = (spot, stage, wither = 0) =>
  (SPOTS[spot] ? sproutOf(...SPOTS[spot], stage, wither) : []);
...
const SINK_OCC = {                                            // 싱크 상판 — 개수통은 없다
  'kt-cleaner': [111, 119], 'kt-sink': [99, 107], 'kt-brush': [117, 124], // 병/수도꼭지/솔
};
const TABLE_OCC = {
  'kt-pot': [52, 72], 'kt-board': [44, 52], 'kt-ingredients': [73, 85],
};
...
// 상판에 앉은 돌도 막는다 — 이제 싱크 상판이다
const ORB_OCC = { sink: [[34, 36, 85, 96]] };
...
export function windowPool(slot, alphaSlot, off = null, orb = null) {
  ...
  for (const { y0, y1, op, x: [sx0, sx1], occ } of SURFACES) {
    for (let y = y0; y <= y1; y++) {
      ...
      if (orb && ORB_OCC[orb] && occ === SINK_OCC)
...
const FEET = {
  ...
  'kt-shoes': [[8, 6, 2, 0.8, 57], [15, 6, 2, 0.8, 57]],
  'kt-umbrella': [[5, 5, 2, 0.7, 52]],
  'kt-lunchbox': [[43, 11, 2, 0.75, 64]],              // 피크닉 바구니 — 바닥으로 내려왔다
...
// [소품, x0, 폭, 밑변 y, 길이, 세기]
const SURF_FEET = {
  'kt-pot':         [[59, 12, 38, 2, 0.75]],       // 작업대 위 — 냄비 밑동
  'kt-board':       [[47, 6, 39, 2, 0.6]],
  'kt-ingredients': [[77, 8, 39, 2, 0.7]],
  'kt-cleaner':     [[114, 5, 35, 1, 0.6]],        // 싱크 상판 — 병(스펀지는 개수통 안)
  'kt-brush':       [[120, 5, 35, 1, 0.5]],
...
const STOVE = { CX: 63.5, CY: 39.2, RX: 10, RY: 4.2 };   // 냄비를 줄인 만큼 화구도 좁힌다
const STOVE_RINGS = (() => {
...
// ※ 셰이프는 손작화 냄비(몸통 x58~69, 귀까지 x55~72, 밑변 y38) 기준.
...
const POT_GLOW = [
  [61, 38, 6, '#ffcf8c', 0.30], [60, 38, 1, '#ffb45c', 0.18], [67, 38, 1, '#ffb45c', 0.18],
