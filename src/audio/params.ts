...
export const MODEL_FIELDS: Record<ModelKind, readonly Field[]> = {
  ...
  burst: [
    ...
    n('durMin', '길이 최소', 0.005, 2, 0.005, 's'),
    n('durMax', '길이 최대', 0.005, 2, 0.005, 's'),
...
export const LAYERS: readonly LayerDef[] = [
  // ── 기존 (게임에서 이미 울리는 것) ──
  {
    ...
    status: '기존',
    trigger: '실내 상시',
    note: '다른 모든 레이어의 바닥. 소스를 pink로 바꿔보면 오래 들을 때 덜 피로하다',
    models: [bed({ src: 'brown', freq: 500, gain: 0.05 })],
  },
  {
    id: 'fireplace',
    name: '벽난로 타닥임',
  ...
  },
  {
    ...
    trigger: '비/장대비 + 실내',
    note: '녹음 예정(todo #2)',
    models: [bed({ freq: 900, gain: 0.03, lfoRate: 0.05, lfoDepth: 0.27 })],
  },
  {
    id: 'rainHard',
    name: '빗속',
    ...
    note: '녹음 예정(todo #3)',
    models: [
      bed({ src: 'white', freq: 2400, gain: 0.045 }),
      burst({
        freq: 1650,
        q: 0.87,
        durMin: 0.02,
  ...
  },
  {
    ...
    note: '녹음 예정(todo #4)',
    models: [
      bed({ src: 'white', freq: 1400, gain: 0.025 }),
      burst({
        freq: 3500,
        q: 1.03,
        durMin: 0.015,
  ...
  },
  {
    ...
    status: '전환',
    trigger: '여름 낮·황혼',
    note: '소스를 white → lfsrShort로 바꾸는 게 이번 전환의 핵심. 재생속도가 음높이다',
    models: [
      buzz({
        src: 'white',
        freq: 5300,
  ...
  // ── 분리안 (birdsWind → birds + wind) ──
  {
    ...
    status: '신규',
    trigger: 'walk / sun',
    note: 'sine → 12.5% 듀티 펄스. sine은 배음이 없어 얇고 멀게 들린다',
    models: [chirp({})],
  },
  {
    id: 'wind',
    ...
    status: '신규',
    trigger: 'walk / sun / 강풍',
    note: '세기가 날씨에 연속 반응해야 해서 녹음보다 합성이 맞다',
    models: [bed({ freq: 420, gain: 0.035, lfoRate: 0.09, lfoDepth: 0.51 })],
  },
  ...
  // ── 신규 칩튠 ──
  {
    ...
    status: '신규',
    trigger: '여름·가을 밤',
    note: '최우선 — 지금 밤은 roomBase 단독이다. 게이트가 트릴을 만든다',
    models: [
      buzz({
        src: 'lfsrShort',
        rate: 1.6,
  ...
  },
  {
    ...
    note: '사각파로 하면 종이 아니라 오르간이 된다 — 비조화 부분음(1 : 2.76 : 5.40)',
    models: [bell({})],
  },
  {
    id: 'typing',
  ...
  },
  {
    ...
    trigger: 'soda 사용 직후',
    models: [
      bed({ src: 'white', filter: 'highpass', freq: 4000, gain: 0.012 }),
      burst({
        freq: 6000,
        q: 4,
        durMin: 0.008,
  ...
  // ── 신규 노이즈 ──
  {
    ...
    status: '신규',
    trigger: 'fan 보유 + 여름',
    note: '회전 주기가 정확해야 해서 합성이 유리. 모터 험(drone) + 날개(bed의 LFO)',
    models: [
      { kind: 'drone', wave: 'sine', duty: 0.5, freq: 62, gain: 0.02, lfoRate: 0, lfoDepth: 0, lfoTarget: 'gain' },
      bed({ freq: 800, gain: 0.03, lfoRate: 8, lfoDepth: 0.4 }),
    ],
  },
  {
    id: 'winterDraft',
    ...
    status: '신규',
    trigger: '겨울',
    note: 'LFO를 gain이 아니라 filter에 걸어야 휘파람이 된다 — Q를 높게',
    models: [
      bed({
        src: 'white',
        filter: 'bandpass',
  ...
  },
  {
    ...
    models: [bed({ src: 'pink', freq: 700, gain: 0.02 })],
  },
];
