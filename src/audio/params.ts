...
export const LAYERS: readonly LayerDef[] = [
  // ── 기존 (게임에서 이미 울리는 것) ──
  {
    ...
    status: '기존',
    trigger: '실내 상시',
    note: '작업용 확정: pink + lp2500 — 말소리 대역(1~3kHz)까지 가리고 오래 들어도 덜 피로하다. 이전 값은 brown+lp500',
    models: [bed({ src: 'pink', freq: 2500, gain: 0.035 })],
  },
  {
    id: 'fireplace',
    name: '벽난로 타닥임',
  ...
  },
  {
    ...
    status: '전환',
    trigger: '여름 낮·황혼',
    note: 'LFSR 전환 완료 — 주기 93 버즈가 팀발의 주기 펄스열에 가깝다. 집중 중에도 울리므로 질감 레벨(저게인·얕은 트레몰로)로 눌렀다',
    models: [
      buzz({
        src: 'lfsrShort',
        rate: 1.2,
  ...
  // ── 분리안 (birdsWind → birds + wind) ──
  {
    ...
    status: '신규',
    trigger: 'walk / sun',
    note: 'sine → 12.5% 듀티 펄스. sine은 배음이 없어 얇고 멀게 들린다. 휴식 장식 레벨로 상향',
    models: [chirp({ volMin: 0.07, volMax: 0.11 })],
  },
  {
    id: 'wind',
    name: '바람 (분리안)',
  ...
  // ── 신규 노이즈 ──
  {
    ...
    status: '신규',
    trigger: 'fan 보유 + 여름',
    note: '재시도 확정 — 험을 배음(55+110Hz)으로 쌓고 날개 변조는 얕게(0.18)',
    models: [
      ...
      bed({ src: 'pink', freq: 1400, gain: 0.011, lfoRate: 15, lfoDepth: 0.18 }),
    ],
  },
  {
    id: 'winterDraft',
